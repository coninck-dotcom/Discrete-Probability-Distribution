...
# Nota: Usamos 99% (0.99) pois a cauda da distribuição F pode ser muito longa
x_max <- qf(0.99, df1 = df1, df2 = df2)
...
dados_f_snedecor <- data.frame(
  x  = x,
  fx = df(x, df1 = df1, df2 = df2), # PDF: Densidade de probabilidade
  Fx = pf(x, df1 = df1, df2 = df2)  # CDF: Probabilidade acumulada
)

# ==============================================================================
# Gráfico da PDF (Função de Densidade de Probabilidade)
# ==============================================================================
g1 <- ggplot(dados_f_snedecor, aes(x = x, y = fx)) +
  ...
  scale_y_continuous(limits = c(0, max(dados_f_snedecor$fx) * 1.1), expand = c(0, 0)) +
  labs(
    title = paste0("Função de Densidade (PDF) - F(df1=", df1, ", df2=", df2, ")"),
    x = "Valor (x)",
    y = "f(x)"
  ) +
  theme_classic() +
...
# ==============================================================================
g2 <- ggplot(dados_f_snedecor, aes(x = x, y = Fx)) +
...
grafico_final 
# Salvar como JPEG
# ggsave("figFSnedecor.jpeg", plot = grafico_final, width = 10, height = 6, dpi = 300)
