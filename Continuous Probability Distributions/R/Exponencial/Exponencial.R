...
dados_exponencial <- data.frame(
  x  = x,
  fx = dexp(x, rate = rate), # PDF: Densidade de probabilidade
  Fx = pexp(x, rate = rate)  # CDF: Probabilidade acumulada
)

# ==============================================================================
# Gráfico da PDF (Função de Densidade de Probabilidade)
# ==============================================================================
g1 <- ggplot(dados_exponencial, aes(x = x, y = fx)) +
  ...
  scale_y_continuous(limits = c(0, max(dados_exponencial$fx) * 1.1), expand = c(0, 0)) +
  labs(
    title = paste0("Função de Densidade (PDF) - Exponencial(rate=", rate, ")"),
    x = "Valor (x)",
    y = "f(x)"
  ) +
  theme_classic() +
...
# ==============================================================================
g2 <- ggplot(dados_exponencial, aes(x = x, y = Fx)) +
...
grafico_final
# Salvar como JPEG
# ggsave("figExponencial.jpeg", plot = grafico_final, width = 10, height = 6, dpi = 300)